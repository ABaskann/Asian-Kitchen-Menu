...
function AllButtons(){
  ...
  <li class="list-inline-item"><button class="button d-inline" id="All">All</button></li>
  <li class="list-inline-item"><button class="button d-inline" id="Korea">Korea</button></li>
  <li class="list-inline-item"><button class="button d-inline" id="Japan">Japan</button></li>
  <li class="list-inline-item">  <button class="button d-inline" id="China">China</button></li>
  </ul>
  `
...
function foodBlock(items){
  let html = `
  <div class="menu-items col-6">
      <img class="photo" src="${items.img}" alt="${items.title}">
      <div class="menu-info">
        <div class="menu-title">
          <h4>${items.title}</h4>
          <h4>${items.price}</h4>
        </div>
        <div class="menu-text">${items.desc}</div>
      </div>
    </div>  
  `
  return html;
...
function showAllFoods(){
  let allFoods = [];
   menu.forEach(foodinfos => {
    allFoods += foodBlock(foodinfos)})
...
function koreanCategory(){
  ...
  menu.forEach(menuItems =>{
    if(menuItems.category=="Korea"){
      koreanFoods += foodBlock(menuItems)
    }
  })
